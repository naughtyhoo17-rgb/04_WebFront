...
/* 이벤트가 일어나는 대상 => document 자체
  thus document.addEventListener로 접근 */
...
// 화면에 존재하는 key 모두 얻어오기
const keys = document.getElementsByClassName("key");
const keyMap = { 'q': 0, 'w': 1, 'e': 2, 'r': 3 }; // K:V 꼴의 JS 객체

// 키보드 이벤트 핸들러 함수
  /* JS Object에서
  객체명[key] => 매핑된 value 반환
  객체명.key => 매핑된 value 반환 */
function changeKeyColor(e, color) {
  ...
  /* keyMap['q'] => 0
     ...
     keyMap['e'] => 2
     keyMap['r'] => 3
 cf) keyMap['a'] => undefined ; keyMap에 없는 값이 입력된 경우 */
...
/* 
  이벤트가 감지되었을 때 실행해야하는 함수(ex. changeKeyColor) 
   1) 만약 해당 함수에 전달할 인자가 없는 경우 
    => 함수명만 써도 됨 ("keydown", changeKeyColor) 
    => changeKeyColor 함수 선언부의 매개변수에서 이벤트 객체 (e)를 가져다 쓸 수 있음

   2) 만약 해당 함수에 전달할 인자가 있는 경우
    => 익명함수 형태로 작성할 것 (e) => {}
    => 전달인자의 첫 번째 값은 e를 넘겨주기, 두 번째 전달인자부터 내가 전달하고싶은 데이터 나열
...
document.addEventListener("keyup", handleKey); // event 객체는 전달인자 e 없어도 ok
...
function handleKey(e) {
  ...
  // 3항 연산자 => keydown이 true이면 deeppink, false이면 white
}
*/